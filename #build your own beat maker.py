...
load_menu = False
saved_beats = []
file = open('FunDrum/my_beats.txt', 'r')
for line in file:
...
# load in sounds
hi_hat = mixer.Sound('FunDrum/sounds/hi hat.wav')
clap = mixer.Sound('FunDrum/sounds/clap.wav')
crash = mixer.Sound('FunDrum/sounds/crash.wav')
kick = mixer.Sound('FunDrum/sounds/kick.wav')
snare = mixer.Sound('FunDrum/sounds/snare.wav')
tom = mixer.Sound('FunDrum/sounds/tom.wav')
...
def draw_grid(clicks, beat, actives):
    ...
    for i in range(beats):
        for j in range(instruments):
            ...
            rect = pygame.draw.rect(screen, color, [i * ((WIDTH - 205) // beats) + 200, (j * 100) + 5, ((WIDTH - 200) // beats) - 10, ((HEIGHT - 200) // instruments) - 10], 0, 3)

            pygame.draw.rect(screen, gold, [i * ((WIDTH - 210) // beats) + 200, (j * 100), ((WIDTH - 200) // beats), ((HEIGHT - 200) // instruments)], 5, 5)

            pygame.draw.rect(screen, black, [i * ((WIDTH - 210) // beats) + 200, (j * 100), ((WIDTH - 200) // beats), ((HEIGHT - 200) // instruments)], 2, 5)
...
def draw_load_menu(index):
    ...
    loaded_bpm = 0
    pygame.draw.rect(screen, black, [0, 0, WIDTH, HEIGHT])
    menu_text = label_font.render('LOAD MENU: Seleect a Beat to Load', True, white)
    ...
    for beat in range(len(saved_beats)):
        if beat < 10:
            ...
            # print(saved_beats[beat][name_index_start:name_index_end])
        ...
        if event.type == pygame.MOUSEBUTTONUP and not save_menu and not load_menu:
            ...
            if save_menu:
                ...
                if saving_button.collidepoint(event.pos):
                    file = open('FunDrum/my_beats.txt', 'w')
